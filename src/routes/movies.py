...
async def get_paginated_movie(page: int = Query(1, ge=1),
    ...
    return MovieListResponseSchema(movies=movies,
    prev_page=prev_page,
    next_page=next_page,
...
async def create_movie(movie_data: MovieCreateSchema, db: AsyncSession = Depends(get_db)):
    ...
    if existing_movie:
        raise HTTPException(detail=f"A movie with the name '{movie_data.name}' and release date '{movie_data.date}' already exists.", status_code=409)
...
async def retrieve_movie(movie_id: int, db: AsyncSession = Depends(get_db)):
    # movie = await db.scalar(select(MovieModel).where(models.MovieModel.id == movie_id))
    movie = await db.scalar(select(MovieModel).options(
        selectinload(MovieModel.genres),
        selectinload(MovieModel.actors),
        selectinload(MovieModel.languages),
